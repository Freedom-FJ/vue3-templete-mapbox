...
popup-panel(  :height='408' :width="736" :titleHeight="32" :isMapPop="true" )
    template(#title)
        .title-box.flex
            .title-box.text-16 {{isRiverCourse ? (detail.watershedText || '--') : properties.name}}
            .analysis-btn.text-14(@click="goToSiteDetail") 进入详情
            .drink-btn-box(v-if="properties.typeCode === '40'")
                .now-btn.text-14(:class="[!drinkShowType ? 'isCheck' : '']" @click="drinkTypeChange(0)")  最新
                .past-30days-btn.text-14(:class="[drinkShowType ? 'isCheck' : '']" @click="drinkTypeChange(1)")  过去24小时趋势变化
    .main-content
        .common-pop-basic-info
            ...
            .info-item-box
                .info-item.flex-g(v-if="!isRiverCourse")
                    .item-name 所属水系：
                    .item-value {{detail.watershedText || '--'}}
                .info-item.flex-g(v-else)
                    .item-name 断面名称：
                    .item-value {{properties.name || '--'}}
                //- .info-item.flex-g
                //-     .item-name 监测方式：
                //-     .item-value {{detail.monitoringMethodsText || '--'}}
                .info-item.flex-g
                    .item-name 控制级别：
                    .item-value {{detail.controlLevelText || '--'}}
                .info-item(style="display: flex;width: 400px")
                    .item-name(style="display: inline-block;") 主要污染物：
                    .item-value(style="display: inline-block;max-width: 460px;") {{detail.factorMinPollutantStr || '--'}}
            .info-item-box
                .info-item.flex-g
                    .item-name 是否交接断面：
                    .item-value {{props.properties.borderSituation && props.properties.borderSituation !== '001' ? '是' : '否'}}
        .camera-list
        ...
        .bottom-flex.flex-bw-c.mgr16(v-if="properties.typeCode === '40'")
            .common-chart-size-box.drink-box(v-if="drinkShowType")
                CommonChart(
                   :factorList="factorList"
                   :properties="properties"
                )
            .common-bottom-content.drink-box(v-if="!drinkShowType")
                .title-header-str.text-14.mgb16 {{ timeStr }}
                popup-table(:option="colunms" ref="popupTableDom"  :data="detail.pollutants" :height="160" :lineStyle="{ height: '32px'}"  :heightLineStyle="{ height: '28px'}" )
</template>
...
onMounted(() => {
    ...
    data.isRiverCourse = props.properties.isRiverCourse
    init()
    // 加载水基础系统的数据
...
const getBaseBaseWebDetailData = async (typeCode: string, paramsObj: any) => {
    try {
        ...
        if (props.properties.isAnalysis)
            data.timeStr = props.properties.time || useAnalysisStore().analysis.data.dataTimeStr

        else data.timeStr = props.properties.time || '--'
...
const init = async () => {
    ...
    if (monitoringMethods === 1 && !props.properties.isAnalysis) data.timeTypeSelected = 'month'
    let factors = []
    // 站点类型
...
const drinkTypeChange = async (val: number) => {
    if (data.drinkShowType === val) return
    data.drinkShowType = val
}
